		...
		function processFile(file, ext, next) {
			...
			if (files[file].hasOwnProperty('layout')) {
				...
				if (jetpack.exists(tmplPath)) {
					...
					// compile again to replace vars that where set in the partials
					let templatePostData = handlebars.compile(content)
					
					let postContent = templatePostData(data)
					// set new content
					files[file].contents = new Buffer(postContent)
					if(logging) {
